        ...
        .then(
            (data) => {
                if (data.data.statusCode == INTERACTION_OK) {
                    console.log(new Date().toLocaleTimeString() + ' - The motion detection has been notified correctly to the orchestration service!')
                }
            }
        )
        .catch((error) => {
...
app.listen(port, () => {
    console.log(`Input service of client 3 is listening on port ${port}`)
})
